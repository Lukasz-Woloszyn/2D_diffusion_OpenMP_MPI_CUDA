...
void diffuseStep5_OMP(const Grid& input, Grid& output, int numThreads) {
    ...
    for (int y = 0; y < H; y++) {
        for (int x = 0; x < W; x++) {
            ...
            int count = 0;
            int idx = (y * W + x) * 3;
            sumR += in[idx]; sumG += in[idx+1]; sumB += in[idx+2]; count++;
            if (y > 0)     { idx=((y-1)*W+x)*3; sumR+=in[idx]; sumG+=in[idx+1]; sumB+=in[idx+2]; count++; }
            if (y < H-1)   { idx=((y+1)*W+x)*3; sumR+=in[idx]; sumG+=in[idx+1]; sumB+=in[idx+2]; count++; }
            if (x > 0)     { idx=(y*W+(x-1))*3; sumR+=in[idx]; sumG+=in[idx+1]; sumB+=in[idx+2]; count++; }
            if (x < W-1)   { idx=(y*W+(x+1))*3; sumR+=in[idx]; sumG+=in[idx+1]; sumB+=in[idx+2]; count++; }
            int oi = (y*W+x)*3;
            float inv = 1.0f/count;
            out[oi]=sumR*inv; out[oi+1]=sumG*inv; out[oi+2]=sumB*inv;
        }
    }
}
...
void diffuseStep9_OMP(const Grid& input, Grid& output, int numThreads) {
    ...
    for (int y = 0; y < H; y++) {
        for (int x = 0; x < W; x++) {
            float sumR=0, sumG=0, sumB=0; int count=0;
            ...
            int oi=(y*W+x)*3; float inv=1.0f/count;
            out[oi]=sumR*inv; out[oi+1]=sumG*inv; out[oi+2]=sumB*inv;
        }
    }
}
...
int main(int argc, char* argv[]) {
    ...
    if (args.help) { Args::printHelp(argv[0]); return 0; }
    ...
    Grid* cur = &gridA; Grid* nxt = &gridB;

    for (int iter = 1; iter <= args.iterations; iter++) {
        if (args.stencil == 9) diffuseStep9_OMP(*cur, *nxt, args.threads);
        else diffuseStep5_OMP(*cur, *nxt, args.threads);
        ...
        if (iter % 100 == 0) std::cout << "  Iteracja " << iter << "/" << args.iterations << "\n";
    }
