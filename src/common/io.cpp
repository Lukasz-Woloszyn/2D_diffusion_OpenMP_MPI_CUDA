...
void saveGridPPM(const Grid& grid, const std::string& filename) {
    ...
    for (int y = 0; y < grid.height; y++) {
        for (int x = 0; x < grid.width; x++) {
            ...
            uint8_t r = static_cast<uint8_t>(std::clamp(grid.data[idx + 0], 0.0f, 1.0f) * 255.0f);
            uint8_t g = static_cast<uint8_t>(std::clamp(grid.data[idx + 1], 0.0f, 1.0f) * 255.0f);
            uint8_t b = static_cast<uint8_t>(std::clamp(grid.data[idx + 2], 0.0f, 1.0f) * 255.0f);
            file.put(r);
            file.put(g);
            file.put(b);
        }
...
std::string frameFilename(const std::string& dir, int iteration, const std::string& ext) {
    char buf[256];
    snprintf(buf, sizeof(buf), "%s/frame_%05d%s", dir.c_str(), iteration, ext.c_str());
