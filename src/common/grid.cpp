...
void Grid::clear() { std::fill(data.begin(), data.end(), 0.0f); }
...
std::vector<ColorSource> generateDefaultSources(int gridSize) {
  ...
  int radius = std::max(3, gridSize / 50); // punkt ma promień o wielkości 1/50
                                           // rozmiaru siatki, minimum 3 piksele

  // Czerwony - lewy górny
  sources.emplace_back(gridSize / 4, gridSize / 4, Color(1.0f, 0.0f, 0.0f),
                       radius);

  // Zielony - prawy górny
  sources.emplace_back(3 * gridSize / 4, gridSize / 4, Color(0.0f, 1.0f, 0.0f),
                       radius);

  // Niebieski - dolny środek
  sources.emplace_back(gridSize / 2, 3 * gridSize / 4, Color(0.0f, 0.0f, 1.0f),
                       radius);

  // Żółty - centrum
  sources.emplace_back(gridSize / 2, gridSize / 2, Color(1.0f, 1.0f, 0.0f),
                       radius);

  // Magenta - lewy dolny
  sources.emplace_back(gridSize / 4, 3 * gridSize / 4, Color(1.0f, 0.0f, 1.0f),
                       radius);
