...
int main(int argc, char* argv[]) {
    ...
    if (args.help) {
        if (rank == 0) Args::printHelp(argv[0]);
    ...
    if (rank == 0) {
        ...
        memcpy(localA.data() + N*3, fullGrid.data.data(), r0rows * N * 3 * sizeof(float));

        // Wysyłanie do innych procesów
        for (int p = 1; p < numProcs; p++) {
            int pRows = baseRows + (p < remainder ? 1 : 0);
            int pStart = p * baseRows + std::min(p, remainder);
            MPI_Send(fullGrid.data.data() + pStart * N * 3, pRows * N * 3,
                     MPI_FLOAT, p, 0, MPI_COMM_WORLD);
        }
    } else {
        MPI_Recv(localA.data() + N*3, localRows * N * 3,
    ...
    for (int iter = 1; iter <= args.iterations; iter++) {
        ...
        if (prevRank >= 0) {
            MPI_Sendrecv(
                curBuf + N*3, N*3, MPI_FLOAT, prevRank, 0,  // send first data row up
                curBuf, N*3, MPI_FLOAT, prevRank, 1,        // recv halo from above
                MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
        // Wysyłanie dolnego wiersza do następnego procesu
        if (nextRank < numProcs) {
            MPI_Sendrecv(
                curBuf + localRows * N*3, N*3, MPI_FLOAT, nextRank, 1,
                curBuf + (localRows+1) * N*3, N*3, MPI_FLOAT, nextRank, 0,
                MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
        ...
        for (int ly = 1; ly <= localRows; ly++) {
            ...
            for (int x = 0; x < N; x++) {
                ...
                // Centrum
                int idx = (ly*N+x)*3;
                sR+=curBuf[idx]; sG+=curBuf[idx+1]; sB+=curBuf[idx+2]; cnt++;
                ...
                if (args.stencil == 9) {
                    ...
                    for (int dy=-1; dy<=1; dy++) {
                        for (int dx=-1; dx<=1; dx++) {
                            ...
                            if (globalY+dy<0||globalY+dy>=N) continue;
                            idx=(ny*N+nx)*3;
                            sR+=curBuf[idx]; sG+=curBuf[idx+1]; sB+=curBuf[idx+2]; cnt++;
                        }
                    }
                    ...
                    // Szablon 5-punktowy
                    if (globalY > 0)   { idx=((ly-1)*N+x)*3; sR+=curBuf[idx]; sG+=curBuf[idx+1]; sB+=curBuf[idx+2]; cnt++; }
                    if (globalY < N-1) { idx=((ly+1)*N+x)*3; sR+=curBuf[idx]; sG+=curBuf[idx+1]; sB+=curBuf[idx+2]; cnt++; }
                    if (x > 0)        { idx=(ly*N+(x-1))*3; sR+=curBuf[idx]; sG+=curBuf[idx+1]; sB+=curBuf[idx+2]; cnt++; }
                    if (x < N-1)      { idx=(ly*N+(x+1))*3; sR+=curBuf[idx]; sG+=curBuf[idx+1]; sB+=curBuf[idx+2]; cnt++; }
                }

                int oi = (ly*N+x)*3;
                float inv = 1.0f/cnt;
                nxtBuf[oi]=sR*inv; nxtBuf[oi+1]=sG*inv; nxtBuf[oi+2]=sB*inv;
            }
        }

        // Przywrócenie źródeł
        for (const auto& src : sources) {
            int ly = src.y - startRow + 1; // +1 for halo
            if (ly >= 1 && ly <= localRows && src.x >= 0 && src.x < N) {
                int idx = (ly*N+src.x)*3;
                nxtBuf[idx]=src.color.r; nxtBuf[idx+1]=src.color.g; nxtBuf[idx+2]=src.color.b;
            }
        }
        ...
        if (!args.benchmark && (iter % args.saveInterval == 0 || iter == args.iterations)) {
            if (rank == 0) {
                Grid fullGrid(N, N);
                int r0rows = baseRows + (0 < remainder ? 1 : 0);
                memcpy(fullGrid.data.data(), curBuf + N*3, r0rows * N * 3 * sizeof(float));
                ...
                MPI_Send(curBuf + N*3, localRows*N*3, MPI_FLOAT, 0, 2, MPI_COMM_WORLD);
            }
        }
    ...
    if (rank == 0) {
        ...
        MPI_Send(curBuf + N*3, localRows*N*3, MPI_FLOAT, 0, 3, MPI_COMM_WORLD);
    }
