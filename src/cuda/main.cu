  ...
  static Args parse(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
      if (!strcmp(argv[i], "--size") && i + 1 < argc)
        ...
        a.iterations = atoi(argv[++i]);
      else if (!strcmp(argv[i], "--stencil") && i + 1 < argc)
        a.stencil = atoi(argv[++i]);
      else if (!strcmp(argv[i], "--block-size") && i + 1 < argc)
        a.blockSize = atoi(argv[++i]);
      else if (!strcmp(argv[i], "--output") && i + 1 < argc)
        a.output = argv[++i];
      else if (!strcmp(argv[i], "--save-interval") && i + 1 < argc)
        a.saveInterval = atoi(argv[++i]);
...
__global__ void diffuseKernel5_shared(const float *__restrict__ input,
                                      float *__restrict__ output, int W,
                                      int H) {
...
__global__ void diffuseKernel9_shared(const float *__restrict__ input,
  ...
  if (gx < W && gy < H) {
    ...
    tile[li] = input[gi];
    tile[li + 1] = input[gi + 1];
    tile[li + 2] = input[gi + 2];
  }
  if (threadIdx.y == 0) {
  ...
  if ((threadIdx.x == blockDim.x - 1 || gx == W - 1) &&
      (threadIdx.y == blockDim.y - 1 || gy == H - 1)) {
...
// ============================================================
// Funkcje pomocnicze (nie są brane z common)
// ============================================================
...
void savePPM(const float *data, int W, int H, const std::string &filename) {
  ...
  for (int i = 0; i < W * H; i++) {
    ...
    uint8_t g =
        (uint8_t)(std::min(std::max(data[i * 3 + 1], 0.0f), 1.0f) * 255);
    uint8_t b =
        (uint8_t)(std::min(std::max(data[i * 3 + 2], 0.0f), 1.0f) * 255);
    f.put(r);
    f.put(g);
    f.put(b);
  }
...
std::string frameName(const std::string &dir, int iter,
                      const std::string &ext) {
  char buf[256];
  snprintf(buf, 256, "%s/frame_%05d%s", dir.c_str(), iter, ext.c_str());
  return buf;
}
...
int main(int argc, char *argv[]) {
    ...
    for (int dy = -radius; dy <= radius; dy++) {
      for (int dx = -radius; dx <= radius; dx++) {
        ...
        if (dist <= radius) {
          ...
          int idx = (ny * N + nx) * 3;
          hostData[idx] = s.r * fac;
          hostData[idx + 1] = s.g * fac;
          hostData[idx + 2] = s.b * fac;
        }
      }
    }
  }
  ...
  cudaMemcpy(d_gridA, hostData.data(), dataSize * sizeof(float),
             cudaMemcpyHostToDevice);
  cudaMemcpy(d_sources, srcVec.data(), srcVec.size() * sizeof(ColorSource),
             cudaMemcpyHostToDevice);
  ...
  cudaMemcpy(d_gridB, d_gridA, dataSize * sizeof(float),
  ...
  for (int iter = 1; iter <= args.iterations; iter++) {
    ...
    applySourcesKernel<<<(numSrc + 255) / 256, 256>>>(nxtDev, d_sources, numSrc,
                                                      N);
    std::swap(curDev, nxtDev);

    if (!args.benchmark &&
        (iter % args.saveInterval == 0 || iter == args.iterations)) {
      cudaMemcpy(hostData.data(), curDev, dataSize * sizeof(float),
  ...
  printf("\nCSV: cuda,%d,%d,%d,%d,%.6f\n", N, args.iterations, args.stencil, BS,
         elapsed);
